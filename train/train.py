...
def main() -> None:
    ...
    # --- Pilotage par variables d'environnement (cf. workflow GitHub Actions) ---
    ...
    # Anti-oversubscription : en mode grid search, la parallélisation se fait
    # au niveau des (fold x combinaison) via GridSearchCV(n_jobs=-1). Si en
    # plus chaque forêt réclame tous les cœurs (n_jobs=-1), on se retrouve
    # avec N_tasks * N_cpu threads qui se battent pour les mêmes cœurs
    # -> plus lent que du séquentiel.
    # En mode "fixed" il n'y a qu'un seul fit : c'est alors la construction
    # des arbres eux-mêmes qu'on parallélise.
    rf_n_jobs = int(os.getenv("RF_N_JOBS", "1" if tune else "-1"))
    ...
    with mlflow.start_run(run_name=f"ec2_rf_{'tuned' if tune else 'fixed'}") as run:
        start = time.time()
        ...
        if tune:
            ...
            # n_jobs=-1 : les 12 combinaisons x 5 folds sont réparties sur
            # tous les cœurs de la machine (backend joblib par défaut).
            search = GridSearchCV(
            ...
            # Mode rapide (CI / démo) : entraînement direct avec HP fournis.
            pipe.set_params(
        ...
        # Un Pipeline sklearn standard : rechargeable par l'API de prod telle quelle.
